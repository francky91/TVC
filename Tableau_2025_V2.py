...
class TournamentBracket:
    ...
    def load_tournament(self, Tableau):
        ...
        '''
        # Ajouter le bouton "Refresh" dans la fenêtre
        refresh_button = ttk.Button(new_window, text="Refresh", command=lambda: self.refresh_tableau(Tableau, canvas))
    ...
    def handle_enter(self, event, entries, player1, player2, level, index_match, tableau, nb_tops, intertop):
        ...
        if current_index < len(entries):
            ...
            if ( (sets_player1 == 2 and mode == "2 sets") or
                ...
                self.refresh_tableau(self.current_table, self.ok_canvas if self.current_table == "Tournoi OK" else self.ko_canvas)
