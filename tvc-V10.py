...
def create_word_file_with_frames(doc, groupes, is_top, position=0, table=None):
    ...
    for i, (titre, joueurs) in enumerate(groupes.items() if is_top else enumerate(groupes, start=1)):
        ...
        ecrire_poule_on_frame(cell, titre, joueurs, is_top)
...
def ecrire_document_word(fichier_sortie, feuille_source_resultat, top_poules, poules):
    ...
    position, table = create_word_file_with_frames(doc, top_poules, is_top=True)

    # Écrire les poules normales à partir de la dernière position et table
    position, _ = create_word_file_with_frames(doc, poules, is_top=False, position=position, table=table)
...
if __name__ == "__main__":
    ...
    fichier_entree = 'C:\\Users\\franck\\TVC\\TVC2425.xlsx'
