...
def open_app(category, round_number, app_type):
...
def on_open_button_click(action):
    ...
    if action == "poules":
        open_app(selected_category, selected_round, "poules")
    elif action == "tableaux":
        open_app(selected_category, selected_round, "tableaux")

# Création de la fenêtre principale
root = tk.Tk()
root.title("Gestion des Poules et Tableaux")
...
open_poules_button = ttk.Button(button_frame, text="Ouvrir Poules", command=lambda: on_open_button_click("poules"))
open_poules_button.pack(side="left", padx=5)

# Bouton "Ouvrir Tableau"
open_tableau_button = ttk.Button(button_frame, text="Ouvrir Tableau", command=lambda: on_open_button_click("tableaux"))
