...
def charger_joueurs(fichier_entree, feuille_source_resultat):
    ...
    for col in ["Tour1", "Tour2", "Tour3", "Tour4"]:
        if col not in joueurs.columns or joueurs[col].isnull().all():
            joueurs[col] = ""
    print(joueurs.columns)
    return joueurs    

'''def filtrer_et_trier_joueurs(joueurs, nb_poules_tops):
    # mapping = (colonne de participation, libellé, critère principal, liste des colonnes de points)
    mapping = [
    ...
    for col, msg, sort_col, liste_col_points in mapping:
        if joueurs[col].str.contains('X', case=False, na=False).any():
            ...
            if sort_col == "points" and len(liste_col_points) > 0:
                ...
                # Étape A: tri par points desc
                stepA = filtered.sort_values(by="points", ascending=False)
                if len(stepA) >= threshold:
                    # Vérifions si la place threshold est déterminée
                    # i.e. le (threshold)ème joueur a STRICTEMENT moins de points
                    # que le (threshold-1)ème
                    if stepA.iloc[threshold - 1]["points"] > stepA.iloc[threshold]["points"]:
                        return stepA, col.lower(), nb_joueurs

                # Étape B: ajouter max_points desc
                stepB = filtered.sort_values(by=["points", "max_points"], ascending=[False, False])
                if len(stepB) >= threshold:
                    if (stepB.iloc[threshold - 1]["points"] > stepB.iloc[threshold]["points"]) \
                       or (stepB.iloc[threshold - 1]["points"] == stepB.iloc[threshold]["points"] and
                           stepB.iloc[threshold - 1]["max_points"] > stepB.iloc[threshold]["max_points"]):
                        return stepB, col.lower(), nb_joueurs

                # Étape C: ajouter la date de naissance, du plus jeune au plus âgé
                # => date la plus récente en premier => ascending=False
                if "naissance" in filtered.columns:
                    stepC = filtered.sort_values(
                        by=["points", "max_points", "naissance"],
                        ascending=[False, False, False]
                    )
                    return stepC, col.lower(), nb_joueurs
                else:
                    # Pas de colonne naissance => on s'arrête à stepB
                    return stepB, col.lower(), nb_joueurs
            else:
                # Tri simple (Tour1, ou nb_poules_tops=0)
                filtered_simple = filtered.sort_values(by=sort_col, ascending=False)
                return filtered_simple, col.lower(), nb_joueurs

    # Si aucun 'X' trouvé, on renvoie le DataFrame original
