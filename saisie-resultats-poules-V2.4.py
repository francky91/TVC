...
class ResultEntryApp:
    ...
    def reset_inputs_on_mode_change(self):
        """Réinitialise les champs d'entrée en fonction du mode global des sets."""
        mode = self.sets_mode_global.get()
        desired_num_sets = 3 if mode == "2 sets" else 5
        ...
        for entries, _, _, _ in self.result_entries:
            # Ajouter des champs si nécessaire
            idx = 1
            for entry in entries:
                if idx > desired_num_sets:
    ...
    def updateTableauWithHoles(self, tableau, niveauBase, is_level_ok):
        ...
        def addPlayerInTab(niveau, matchId, joueur, is_level_ok):
            ...
            if (top_count == 2 and is_level_ok):
                ...
                newMatchId = matchId
                print("top_count == 2 and is_level_ok, newMatchId:", newMatchId)
        ...
        while (currentNiveau >=4 and found_holes >= 1):
            ...
            for match_id, players in list(tableau.get(label_ok, {}).items()):
                ...
                print ("In holes, joueur1", joueur1, ",joueur2", joueur2, ",match_id:", match_id)
                is_level_ok = False
                if (joueur1 == "---"):
                    print ("J1 = ---, joueur1", joueur1, ",joueur2", joueur2)
                    addPlayerInTab(niveauDown, int(match_id), joueur2, is_level_ok)
                    found_holes += 1
                elif (joueur2 == "---"):
                    print ("J2 = ---, joueur1", joueur1, ",joueur2", joueur2)
