...
def show_points_window(app):
    ...
    def save_points_in_github():
        ...
        categorie = app.tournoi.category
        wb_github = load_workbook(temp_file)
        ws_github =  wb_github[categorie]
        wb_local = load_workbook(fichier_entree)
        ws_local = wb_local[categorie]

        for row in ws_github.iter_rows():
            for cell in row:
        ...
        for row in ws_local.iter_rows():
            for cell in row:
                if isinstance(cell, MergedCell):
                    continue
                ws_github[cell.coordinate].value = cell.value

        wb_github.save(temp_file)
