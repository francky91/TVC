...
def update_set_visibility(mode, entries, max_sets=5):
    ...
    if mode == "2 sets":
        ...
        desired_num_sets = 5
    elif mode in ["Forfaitj1", "Forfaitj2"]:
        desired_num_sets = 0
    ...
    for i, entry in enumerate(entries):
        if i >= desired_num_sets:
            ...
            entry.pack(side="left", padx=2)  # Affiche les champs nécessaires
            entry.configure(state="normal")  # Optionnel pour activer
                      
def clear_set_data_one_match(entries):
    """Efface les champs d’un match."""
...
def handle_mode_change(current_mode, new_mode, result_entries, results_modified):
    ...
    for entries in result_entries:
        clear_set_data_one_match(entries)
