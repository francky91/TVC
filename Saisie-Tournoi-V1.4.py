...
class TournamentBracket:
    ...
    def handle_enter(self, event, entries, player1, player2, level, index_match, tableau, nb_tops, intertop):
        ...
        def set_new_player(level, idx, player, nb_tops, intertop=False):
            print(f"In set_new_player, player: {player}, intertop: {intertop}")
            new_level = ""
            try:
                if (not intertop):
    ...
    def draw_bracket(self, tableau, firstTour, nb_players_top):
        ...
        while (nb_matches >=1):
            ...
            for round_mid_y in next_round_positions:
                ...
                def display_entree(round_mid_y, text):
                    ...
                    if (text == "---"):
                        fillColor = "grey"
                        text = ""
                    self.canvas.create_rectangle(start_x, y_top, start_x + longueur_rectangle, y_bottom, outline="black", width=1, fill=fillColor)
                    if (text != ""):
                        self.canvas.create_text(start_x + 3, y_top + 8, text=text, anchor="w", font=("Arial", 6))  # Réduction de la taille du texte
                    return y_top, y_bottom, mid_y

                if (index_player == 1):
                    if key in matches_list and "joueur1" in matches_list[key]:
                        j1 = matches_list[str(idx)]['joueur1']
                    y1_top,y1_bottom,mid_y1 = display_entree(round_mid_y, j1)
                    ...
                    y2_top,y2_bottom,mid_y2 = display_entree(round_mid_y, j2)       
                    mid_y = (mid_y1 + mid_y2) // 2
                    index_player = 1
                    ...
                    table_label_x = button_x + 40
                    table_label = tk.Label(self.canvas, text="Table")
                    self.canvas.create_window(table_label_x, button_y, window=table_label)

                    table_entry_x = table_label_x + 35

                    def validate_table_input(new_text):
                        """Valide que l'entrée est soit vide, soit un entier entre 1 et 20."""
                        if new_text == "":
                            return True
                        if new_text.isdigit():
                            val = int(new_text)
                    ...
                    validate_cmd = self.canvas.register(validate_table_input)

                    table_var = tk.StringVar(value="")  # Valeur par défaut (vide)
                    ...
                    self.canvas.create_window(table_entry_x, button_y, window=table_entry)
