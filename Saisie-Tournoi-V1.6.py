...
class TournamentBracket:
    ...
    def print_bracket(self):
        ...
        # 1) Déterminer la zone totale occupée par les dessins du Canvas
        left, top, right, bottom = self.canvas.bbox("all")
        width_ps  = right - left
        height_ps = bottom - top

        # Ajouter une **marge à gauche** (par exemple 100 pixels)
        margin_left = 100
        left -= margin_left
        width_ps += margin_left

        # Vérifier des dimensions valides
        if width_ps <= 0 or height_ps <= 0:
            messagebox.showerror("Impression", "Rien à imprimer ou dimensions invalides.")
            return

        ps_file = "bracket_temp.ps"
        png_file = "bracket_temp.png"
        png_file_part1 = "bracket_temp_part1.png"
        png_file_part2 = "bracket_temp_part2.png"

        # 2) Exporter en PostScript avec marge
        try:
        ...
        # 3) Convertir PostScript → PNG avec **mise à l’échelle et DPI**
        try:
            img = Image.open(ps_file)

            # Appliquer un **facteur d'agrandissement**
            scale_factor = 2.0
        ...
        try:
            ...
            img_part1 = img.crop((0, 0, width, half_height))  # Partie haute
            img_part2 = img.crop((0, half_height, width, height))  # Partie basse

            img_part1.save(png_file_part1, "PNG", dpi=(300, 300))
            img_part2.save(png_file_part2, "PNG", dpi=(300, 300))
        except Exception as e:
            messagebox.showerror("Impression", f"Échec de la découpe : {e}")
            return

        # 5) Envoi à l’imprimante **page par page**
        current_os = platform.system()
        try:
            if current_os == "Windows":
                print("print page 1")
                os.startfile(png_file_part1, "print")
                time.sleep(10)
                print("print page 2")
                os.startfile(png_file_part2, "print")
            else:
                # macOS / Linux
                subprocess.run(["lpr", png_file_part1])
                time.sleep(2)
                subprocess.run(["lpr", png_file_part2])
        except Exception as e:
            messagebox.showerror("Impression", f"Échec d'impression : {e}")
    ...
    '''def print_bracket(self):
        ...
        # 1) Déterminer la zone totale occupée par le Canvas
        left, top, right, bottom = self.canvas.bbox("all")
        width_ps = right - left
        height_ps = bottom - top
        ...
        # 3) Convertir PostScript en image PNG
        try:
            img = Image.open(ps_file)
            width, height = img.size
            new_width = int(width * 2)  # Agrandir x2 pour la lisibilité
            new_height = int(height * 2)
            img = img.resize((new_width, new_height), Image.Resampling.LANCZOS)
        
            img_with_margins = Image.new("RGB", (new_width + MARGE_GAUCHE, new_height + MARGE_HAUT + MARGE_BAS), "white")
            img_with_margins.paste(img, (MARGE_GAUCHE, MARGE_HAUT))

            img_with_margins.save(png_file, "PNG", dpi=(300, 300))
        except Exception as e:
            messagebox.showerror("Impression", f"Échec de la conversion en PNG : {e}")
            return

        # 4) Diviser l'image en plusieurs pages si nécessaire
        pdf_file = "bracket_final.pdf"
        pdf = FPDF()
    
        img_width, img_height = img_with_margins.size
        pdf_width, pdf_height = 210, 297  # A4 en mm (sans marges)

        # Vérifier si l'image est plus grande qu'une page
        if img_height > 1100:  # ~Hauteur d'une page A4 en pixels
            parts = (img_height // 1100) + 1
            for i in range(parts):
                y_crop = i * 1100
                img_crop = img_with_margins.crop((0, y_crop, img_width, min(y_crop + 1100, img_height)))
                temp_crop_file = f"bracket_part_{i}.png"
                img_crop.save(temp_crop_file, "PNG", dpi=(300, 300))
            
                pdf.add_page()
                pdf.image(temp_crop_file, x=10, y=10, w=190)
                os.remove(temp_crop_file)
        else:
            pdf.add_page()
            pdf.image(png_file, x=10, y=10, w=190)
    ...
    def print_bracket_to_pdf(self):
        ...
        pdf_filename = f"Tableau_{self.category}_Tour{self.round_number}.pdf"
        ...
        ps_filename = "temp_bracket.ps"
        try:
            self.canvas.postscript(file=ps_filename, colormode="color", x=left, y=top, width=width_canvas, height=height_canvas)
        except Exception as e:
            messagebox.showerror("Impression", f"Échec de l'export PostScript : {e}")
            return

        # 3) Convertir PostScript -> Image (PNG) avec une meilleure résolution
        png_filename = "temp_bracket.png"
        try:
            img = Image.open(ps_filename)

            # Augmenter la résolution pour meilleure lisibilité
            scale_factor = 2  # Multiplier la taille par 2 pour plus de netteté
        ...
        try:
            ...
            pdf_canvas.drawImage(png_filename, margin_x, margin_y, width=final_width, height=final_height)
    ...
    def draw_bracket(self, tableau, firstTour, nb_players_top, table_name):
        ...
        def on_click_print():
            self.print_bracket_to_pdf()
        
        btn_print = tk.Button(self.canvas, text="PDF", command=on_click_print)
        # Placez-le, par exemple, en haut à droite du Canvas :
        self.canvas.create_window(900, 30, window=btn_print)  # ajustez les coords
        ...
        while (nb_matches >=1):
            ...
            for round_mid_y in next_round_positions:
                ...
                def display_entree(round_mid_y, dossard_text, text, level_str, match_idx):  
                    ...
                    #if (text != ""):
                    text_id = self.canvas.create_text(start_x + 3, y_top + 8, text=text, anchor="w", font=("Arial", 10))  # Réduction de la taille du texte
