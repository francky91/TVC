...
class TournamentBracket:
    ...
    def handle_enter(self, event, entries, player1, player2, level, index_match, tableau, nb_tops, intertop):
        ...
        self.results_modified = [True]
        mode = self.sets_mode_global.get()
        table_intertops = [(7,3), (6, 2), (5, 4), (8, 1)]
        ...
        def set_new_player(level, idx, player, nb_tops, dossard, intertop=False):
            new_level = ""
            try:
                if (not intertop):
        ...
        if current_index < len(entries):
            ...
            def gestion_vainqueur(player1, player2, dossard1, dossard2):
                if (intertop):
                    set_new_player(level, table_intertops[index_match][0], player1, nb_tops, dossard1, intertop)
                    set_new_player(level, table_intertops[index_match][1], player2, nb_tops, dossard2, intertop)
                    if (index_match==0):
                    ...
                    set_new_player(level, index_match, player1, nb_tops, dossard1, intertop)
            ...
            if ( (sets_player1 == 2 and mode == "2 sets") or
                ...
                gestion_vainqueur(player1, player2, dossard1, dossard2)   
            elif ( (sets_player2 == 2 and mode == "2 sets") or
                (sets_player2 == 3 and mode == "3 sets") ):                                          
                gestion_vainqueur(player2, player1, dossard2, dossard1)
            else:
        ...
        while (nb_matches >=1):
            ...
            for round_mid_y in next_round_positions:
                ...
                def display_entree(round_mid_y, dossard_text, text, level_str, match_idx):
                    ...
                    #if (text != ""):
                    text_id = self.canvas.create_text(start_x + 3, y_top + 8, text=text, anchor="w", font=("Arial", 9))  # Réduction de la taille du texte
