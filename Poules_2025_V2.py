...
class ResultEntryApp:        
    ...
    def updateTableauWithHoles(self, tableau, niveauBase, is_level_ok):       
        ...
        def addPlayerInTab(niveau, matchId, joueur, dossard, is_top_tour):
            if dossard == "Unknown" and joueur.nom != "---":
                messagebox.showerror(
                    "Erreur de données",
                    f"Le joueur {joueur.nom} {joueur.prenom} ({joueur.club}) a un dossard inconnu. Veuillez vérifier les données."
                )
        ...
        found_holes = 2
        print (f"currentNiveau: {currentNiveau}, found_holes: {found_holes}")
