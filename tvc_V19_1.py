...
def echanger_joueur(poule_surplus, poule_autre, club):
    ...
    Avant de valider l'échange, la fonction vérifie que les comptes pour les deux clubs 
    impliqués (celui en surplus et l'autre) restent équilibrés (différence <= 1).
    
    Retourne True si un échange a été effectué sans créer de déséquilibre, False sinon.
    """
    from collections import defaultdict
    
    # Fonction pour recalculer les comptes pour les deux poules
    def get_counts_for_clubs(pA, pB, clubs):
        counts = defaultdict(lambda: [0, 0])
        for joueur in pA:
            if joueur['club'] in clubs:
                counts[joueur['club']][0] += 1
        for joueur in pB:
            if joueur['club'] in clubs:
                counts[joueur['club']][1] += 1
    ...
    # Indices sur lesquels on a le droit d'agir (on ignore l'indice 0)
    indices_surplus = range(1, len(poule_surplus))
    indices_autre = range(1, len(poule_autre))

    # On parcourt toutes les paires candidates
    for i in indices_surplus:
        j1 = poule_surplus[i]
        if j1['club'] != club:
            continue  # on cherche un joueur qui est bien du club à redistribuer
        ...
        for k in indices_autre:
            ...
            # Calculer les comptes avant l'échange pour les deux clubs concernés
            clubs_to_check = [j1['club'], j2['club']]
            counts_before = get_counts_for_clubs(poule_surplus, poule_autre, clubs_to_check)

            # Effectuer le swap temporaire
            poule_surplus[i], poule_autre[k] = j2, j1

            # Calculer les comptes après l'échange
            counts_after = get_counts_for_clubs(poule_surplus, poule_autre, clubs_to_check)

            # Vérifier que pour chacun des clubs concernés, la différence est <= 1
            valid_swap = True
            for club_check in clubs_to_check:
                diff = abs(counts_after[club_check][0] - counts_after[club_check][1])
                if diff > 1:
                    valid_swap = False
                    break

            if valid_swap:
                # Swap validé, on renvoie True
                return True
            else:
                # Revenir sur l'échange (swap inverse)
                poule_surplus[i], poule_autre[k] = j1, j2

    # Si aucun échange n'est possible sans déséquilibre, retourner False
    return False
...
def reequilibrer_2_poules(pouleA, pouleB):
    ...
    while True:
        ...
        print(f"club_counts: {club_counts}")
        ...
        for club, (countA, countB) in club_counts.items():
            if abs(countA - countB) > 1:
                club_trouve = club
                print(f"club_trouve: {club_trouve}")
                break
...
def repartir_poules_sans_conflit(fichier_entree, feuille_source_resultat, nb_poules_top, nb_poules_3=None, nb_poules_4=None):
