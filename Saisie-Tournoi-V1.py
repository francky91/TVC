...
class TournamentBracket:
    ...
    def handle_enter(self, event, entries, player1, player2, set_choice, level, index_match, tableau, intertop):
        ...
        def set_new_player(level, idx, player, intertop=False):
            new_level = ""
            try:
                if (not intertop):
            ...
            if (not intertop):
                if (int(current_level) == 8*self.nb_tops):
                    index_table = str(idx)
                else:
        ...
        if current_index < len(entries) - 1:
            ...
            if (int(mode) == 2 and (sets_player1 >= 2 or sets_player2 >= 2)) or \
                ...
                if (sets_player1 >= 2):                        
                    ...
                    if (intertop):
                        set_new_player(level, table_intertops[index_match][0], player1, intertop)
                        set_new_player(level, table_intertops[index_match][1], player2, intertop)
                    else:
                        set_new_player(level, index_match, player1, intertop)                        
                else:
                    print("winner found: ", player2)
                    if (intertop):
                        set_new_player(level, table_intertops[index_match][0], player2, intertop)
                        set_new_player(level, table_intertops[index_match][1], player1, intertop)
                    else:   
                        set_new_player(level, index_match, player2, intertop)
    ...
    def open_match_window(self, player1, player2, string_level, index_match, tableau, intertop=False):
        # Créer une nouvelle fenêtre
        print("string_level:", string_level, ",index_match:", index_match)
        ...
        for i in range(5):
            ...
            entry.bind("<Return>", lambda event, e=set_entries, p1=player1, p2=player2, lvl=string_level, idx=index_match: self.handle_enter(event, e, p1, p2, set_choice, lvl, idx, tableau, intertop))
        ...
        # Bouton pour valider
        validate_button = ttk.Button(button_frame, text="Valider", command=self.save_results)
        validate_button.pack(side="left", padx=5)
    ...
    def draw_intertops(self, tableau):
        ...
        for idx in range (0, 4):
            ...
            state = "normal"
            intertop = True
            btn = tk.Button(self.canvas, text="Détails", state=state, command=lambda p1=joueur1, p2=joueur2, lvl="InterTops", match_index=idx: self.open_match_window(p1, p2, lvl, match_index, tableau, intertop))
            ...
            for round_mid_y in next_round_positions:
                ...
                if (index_player == 1):
                    ...
                    if (j1 == "" or j1 == "---" or j2 == "" or j2 == "---"):
                        state="disabled"
                    btn = tk.Button(self.canvas, text="Détails", state = state, command=lambda p1=j1, p2=j2, lvl=string_level, match_index=idx: self.open_match_window(p1, p2, lvl, match_index, tableau))
                    self.canvas.create_window(button_x, button_y, window=btn)
