...
class ResultEntryApp:        
    ...
    def save_tableaux(self):
        ...
        tableau_ok_clean = [list(entry.values())[0] for entry in self.tableauOk]
        tableau_ko_clean = [list(entry.values())[0] for entry in self.tableauKo]
        print("tableauko_clean: ", tableau_ko_clean)
        print("self.TABLEAUKO", self.tableauKo)
        ...
        '''def create_list_match(tableau_clean, joueurs_info):
            ...
            for i in range(0, len(tableau_clean), 2):
                joueur1_nom = tableau_clean[i]
                joueur2_nom = tableau_clean[i + 1] if i + 1 < len(tableau_clean) else ""
                #print("joueur1_nom: ", joueur1_nom, "joueur2_nom: ", joueur2_nom, "i: ", i, ",len(tableau_clean): ", len(tableau_clean))
                ...
                #print (f"list_match[{match_index}]: ", list_match[match_index])
                idx += 2
            return list_match
        '''
        
        def create_list_match(tableau, joueurs_info, nb_top_players):
            """
        ...
        list_match_ok = create_list_match(self.tableauOk, self.joueurs_info, nb_top_players)
        list_match_ko = create_list_match(self.tableauKo, self.joueurs_info, nb_top_players)
        ...
        #tableau_ok_data = existing_data.get("Tableau OK", {})
        ...
        #tableau_ko_data = existing_data.get("Tableau KO", {})
