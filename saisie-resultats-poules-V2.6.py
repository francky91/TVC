...
class ResultEntryApp:        
    ...
    def confirm_poule_change(self, event):
        ...
        if self.results_modified:
            ...
            if not response:
                ...
                print("current_poule:", self.current_poule)
    ...
    def handle_enter(self, event, entries, player1, player2, victory_label):
        ...
        if current_index <= len(entries) - 1:
            ...
            print("mode:", mode, ",sets_player1:", sets_player1, ",sets_player2:", sets_player2)
        ...
        while (currentNiveau >=4 and found_holes >= 1):
            ...
            for match_id, players in list(tableau.get(label_ok, {}).items()):
                ...
                if (niveauDown == self.top_count*4):
                    is_top_tour =True
                if (joueur1 == "---"):
                    addPlayerInTab(niveauDown, int(match_id), joueur2, is_top_tour)
                    found_holes += 1
                elif (joueur2 == "---"):
                    addPlayerInTab(niveauDown, int(match_id), joueur1, is_top_tour)
                    found_holes += 1
    ...
    def save_tableaux(self):
        ...
        # Calcul de l'index
        print("save_tableaux, classique_count:", self.classique_count)
        print("tableauOk: ", self.tableauOk)
    ...
    def count_poule_types(self):
        ...
        self.classique_count = 0
        for poule_name, poule_data in self.match_data.items():
            print("poule name:", poule_name, " poule_data:", poule_data)
            if "Top" in poule_name:
                self.top_count += 1
            elif "Poule" in poule_name:
                self.classique_count += 1
        print("top_count:", self.top_count, " classique_count:", self.classique_count)
