...
class TournamentBracket:
    ...
    def show_points_window(self):
        ...
        def save_points():
            ...
            self.save_points_in_excel(new_points)
        ...
        while (nb_matches >=1):
            ...
            for round_mid_y in next_round_positions:
                ...
                def on_checkbox_change(var, level, match_index, tableau, window):
                    ...
                    matchOkTmp = self.matchOk
                    self.matchOk = True
                    self.save_results()
                    self.matchOk = matchOkTmp
    ...
    def on_print_and_uncheck(self, player1, player2, table_name, category, level):
        ...
        checked_matches = [
         (tab, level, match_index)
            for (tab, level, match_index), var in self.chk_print_vars.items()
            if var.get() and tab==table_name
        ]

        print("table_name:", table_name, " checked_matches:", checked_matches)
        ...
        # Si plusieurs cases sont cochées, afficher tous les matchs
            ...
            matches = []
            for tab, level, match_index in checked_matches:
                if table_name == "OK":
        ...
        for (tab, level, match_index), var in list(self.chk_print_vars.items()):
            if tab == table_name:
                var.set(False)
