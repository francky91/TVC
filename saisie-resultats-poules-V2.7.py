...
class ResultEntryApp:        
        ...
        for poule_name, poule_data in self.match_data.items():
            for player in poule_data.get("joueurs", []):
                ...
                self.joueurs_info[full_name] = {
                    ...
                    "dossard": player.get("dossard", "Unknown")  # Vérifie que le dossard est bien présent
                }
                print("FULL_NAME:", full_name, ", joueurs_info:", self.joueurs_info[full_name])
        ...
        if (type_poule == "Classique"):
            ...
            for entries, player1, player2, victory_label in self.result_entries:
                match_result = {
                    "player1": player1,
                    "player2": player2,
                    #"dossard_joueur1": player1["dossard"],  # Ajout du dossard du joueur 1
                    #"dossard_joueur2": player2["dossard"],  # Ajout du dossard du joueur 2
    ...
    def updateTableauWithHoles(self, tableau, niveauBase, is_level_ok):       
        ...
        def addPlayerInTab(niveau, matchId, joueur, dossard, is_level_ok):
            label_niveau = f"1/{niveau}"
            if label_niveau not in tableau:
                tableau[label_niveau] = {}
            if (self.top_count == 2 and is_level_ok):                
                newMatchId = matchId
        ...
        while (currentNiveau >=4 and found_holes >= 1):
            ...
            for match_id, players in list(tableau.get(label_ok, {}).items()):
                ...
                if "joueur1" in players:
                    ...
                    print("JOUEUR 1", players["joueur1"])
                if "joueur2" in players:
                    joueur2 = players["joueur2"]
                    print("JOUEUR 2", players["joueur2"])
                is_top_tour = False
                if (niveauDown == self.top_count*4):
                    is_top_tour =True
                if (joueur1 == "---"):
    ...
    def save_tableaux(self):
        ...
        def create_list_match(tableau_clean, joueurs_info):
            list_match = {}
            idx = 0
            for i in range(0, len(tableau_clean), 2):
                joueur1_nom = tableau_clean[i]
                joueur2_nom = tableau_clean[i + 1] if i + 1 < len(tableau_clean) else ""
                #print("joueurs_info: ", joueurs_info)
                ...
                print("J1: ", joueur1_nom, " ", joueurs_info.get(joueur1_nom, {}))
                match_index = 1 + idx // 2
                list_match[match_index] = {
                    "joueur1": joueur1_nom,
                    "dossard_joueur1": joueur1_dossard,  # Ajout du dossard
                    "joueur2": joueur2_nom,
                    "dossard_joueur2": joueur2_dossard  # Ajout du dossard
                }
                idx += 2
            return list_match
        self.joueurs_info = {player["nom"] + " " + player["prenom"] + " " + player["club"]: player for poule in self.match_data.values() for player in poule.get("joueurs", [])}  
        list_match_ok = create_list_match(tableau_ok_clean, self.joueurs_info)
        list_match_ko = create_list_match(tableau_ko_clean, self.joueurs_info)
        ...
        if (type_poule == "Top"):
            if (self.top_count == 2):
                ...
                for player in final_ranking:
                    ...
                    print("interTops=", self.interTops[str(idx)])
                    idx += 1
    ...
    def load_tableau(self):
        ...
        tableau_sauve_OK, tableau_sauve_KO = load_tableau(self.fichier_Tableau)
        ...
        # Extraire les joueurs de tableau_sauve_OKsave_resultats_poules
        joueurs = []
